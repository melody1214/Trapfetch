...
mmap_node *newMNode(int key, struct user_regs_struct *regs, long long timestamp, char *path)
{
	...
	newnode->md = key;
	newnode->path = (char *)malloc(path_len);
	newnode->timestamp = timestamp;
	newnode->next = NULL;
	newnode->prev = NULL;


	if ((lba = get_block_address(path, regs->ebp)) < 0)
		return NULL;

	newnode->mmap_start = (void *)regs->eax;
	newnode->mmap_end = (void *)(regs->eax + regs->ecx);
	newnode->mmap_len = regs->ecx;
	newnode->mmap_off = regs->ebp;
	newnode->lba = lba;
