...
static void startup_child(int argc, char **argv) {
	...
	*/
	...
	if (pid == 0) {
		...
		setenv("LD_LIBRARY_PATH", "/home/melody/study/project/bp/codes/melody/prefetch/bin/", 1);
		setenv("LD_PRELOAD", "/home/melody/study/project/bp/codes/melody/prefetch/bin/libwrapper_32.so", 1);
	...
	switch (event) {
		case 0:
			if ((sig == SIGTRAP) || (sig == (SIGTRAP | 0x80))) {
				...
				if ((regs.orig_eax == SYS_mmap2) && ((int)regs.edi >= 3)) {
					if (insyscall == 0) {
						timestamp = get_timestamp();
						insyscall = 1;
					}
						...
						if (getFilepath(traced_process, (int)regs.edi, fname) == 0) {
							logical_block_addr = get_block_address(fname, regs.ebp);
