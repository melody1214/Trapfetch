...
etc_node *get_trigger(burst_group *b_group, mmap_list *m_list, circ_queue *q, etc_node *e_node)
{
	...
	if (e_node->timestamp >= q_last_time)
		return NULL;
...
void append_burst_list(circ_queue *q, burst_group *b_group, burst_list *b_list)
{
	burst_node *node, *node_prev, *temp;
	burst_group *temp_group;
	int i;

	temp_group = b_list->group_head;

	while (temp_group != NULL && temp_group != b_group) {
		i = q->front;

		for (; i <= q->rear; i++) {
			temp = copy_from_queue(q, i);
			node = temp_group->node_head;

			while ((node != NULL) && (temp->lba != 0)) {
				if (node->lba == temp->lba) {
					if ((temp->offset < node->offset) && ((temp->offset + temp->length) >= node->offset) && ((temp->offset + temp->length) <= (node->offset + node->length))) {
						q->ele[i].length = node->offset - temp->offset;
					}
						...
						q->ele[i].length = (temp->offset + temp->length) - (node->offset + node->length);
						q->ele[i].offset = node->offset + node->length;
					}
					else if ((temp->offset >= node->offset) && ((temp->offset + temp->length) <= (node->offset + node->length))) {
						q->ele[i].lba = 0;
					}
				}
